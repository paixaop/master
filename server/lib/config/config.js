...
serverConfig.mqtt = {
  ...
  security: {
    ...
    // Limit the maximum size of a MQTT topic. Messages sent on longer topics
    // are ignored
    max_topic_length: 1000,

    // Some parts of the MQTT topic are used as parameters to identify control
    // names or system routes, the following settings configure the validation
    // of said parameters after they are extracted from the topic string.

    // Regular expressions that validate the route parameter extracted from
    // MQTT topic if any of the regular expressions matches then route is
    // considered valid
    valid_routes: [
  ...
  brokers: {
    ...
    mybroker: {
      ...
      // Topic to subscribe to and receive messages, '+' and '#' MQTT wildcards
      // are supported
      topic: "master/#",
